...
data_url = "https://raw.githubusercontent.com/zainzain22/sepeda/refs/heads/main/day.csv"  # Ganti dengan URL data yang sesuai
df = pd.read_csv(data_url)

# Step 2: Buat halaman navigasi untuk kedua pertanyaan
page = st.selectbox("Pilih halaman", ["Pertanyaan 1: Hubungan Season dan Cnt", "Pertanyaan 2: Hubungan Weekday dan Cnt"])

# Step 3: Menampilkan tabel sederhana
def display_table(data):
    st.write(data)

if page == "Pertanyaan 1: Hubungan Season dan Cnt":
    # Halaman untuk Pertanyaan 1
    st.subheader("Tabel Total Jumlah Sepeda yang Dipinjam Berdasarkan Musim (Season)")

    # Mengonversi season menjadi kategori
    season_mapping = {1: 'Winter', 2: 'Spring', 3: 'Summer', 4: 'Fall'}
    df['season'] = df['season'].map(season_mapping)

    # Mengelompokkan data berdasarkan season dan menghitung total cnt untuk setiap season
    season_cnt_total = df.groupby('season')['cnt'].sum().reset_index()

    # Menampilkan tabel dengan total cnt per season
    display_table(season_cnt_total)

    # Definisikan palet warna gradasi dari biru gelap ke biru standar
    gradient_palette_season = sns.color_palette(["#003f5c", "#375a8c", "#7a8dbe", "#bcdef0"])

    # Visualisasi 1: Bar Plot untuk Total Jumlah Sepeda yang Dipinjam berdasarkan Musim
    ...
    # Visualisasi 2: Box Plot untuk Distribusi Jumlah Sepeda yang Dipinjam berdasarkan Musim
    ...
    bins = [0, 100, 200, 300, float('inf')]  # Rentang untuk binning
    labels = ['Rendah', 'Sedang', 'Tinggi', 'Sangat Tinggi']

    # Mengelompokkan cnt ke dalam kategori bin
    df['cnt_binned'] = pd.cut(df['cnt'], bins=bins, labels=labels, right=False)

    # Mengelompokkan data berdasarkan season dan menghitung total cnt
    season_cnt_binned = df.groupby(['season', 'cnt_binned'])['cnt'].sum().unstack().fillna(0)

    # Menampilkan tabel dengan total cnt per season dan kategori bin
    st.subheader("Tabel Jumlah Sepeda yang Dipinjam Berdasarkan Musim dan Kategori")
    display_table(season_cnt_binned)

    # Visualisasi: Heatmap untuk melihat distribusi jumlah sepeda per season dan kategori bin
    ...
    # Halaman untuk Pertanyaan 2
    st.subheader("Tabel Total Jumlah Sepeda yang Dipinjam Berdasarkan Hari (Weekday)")

    # Mengonversi weekday menjadi kategori dengan urutan yang sesuai (Monday - Sunday)
    weekday_mapping = {0: 'Sunday', 1: 'Monday', 2: 'Tuesday', 3: 'Wednesday', 4: 'Thursday', 5: 'Friday', 6: 'Saturday'}
    df['weekday'] = df['weekday'].map(weekday_mapping)
    
    # Mengubah weekday menjadi kategori dengan urutan yang benar
    ordered_days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']
    df['weekday'] = pd.Categorical(df['weekday'], categories=ordered_days, ordered=True)

    # Mengelompokkan data berdasarkan weekday dan menghitung total cnt untuk setiap weekday
    weekday_cnt_total = df.groupby('weekday')['cnt'].sum().reindex(ordered_days).reset_index()

    # Menampilkan tabel dengan total cnt per weekday
    display_table(weekday_cnt_total)

    # Definisikan palet warna gradasi dari hijau gelap ke hijau standar
    gradient_palette_weekday = sns.color_palette(["#004d00", "#006600", "#338a00", "#66b300"])

    # Visualisasi 1: Line Plot untuk Jumlah Sepeda yang Dipinjam berdasarkan Weekday
    ...
    # Visualisasi 2: Box Plot untuk Distribusi Jumlah Sepeda yang Dipinjam berdasarkan Weekday
    ...
    # Menentukan hari dengan jumlah sepeda tertinggi
